    ...
    def _calc_update_factors(self, y_true, y_pred):
        ...
        for attr_name in dir(self.network):
            ...
            if isinstance(attr, Linear):
                self.__update_factors[id(attr.weight)] = (-1 * self.learning_rate) * attr.grads['weight']
                self.__update_factors[id(attr.bias)] = (-1 * self.learning_rate) * attr.grads['bias']
            if isinstance(attr, Sequential):
                for layer in attr.layers:
                    if isinstance(layer, Linear):
                        self.__update_factors[id(layer.weight)] = (-1 * self.learning_rate) * layer.grads['weight']
                        self.__update_factors[id(layer.bias)] = (-1 * self.learning_rate) * layer.grads['bias']
